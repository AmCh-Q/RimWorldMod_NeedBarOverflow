namespace NeedBarOverflow
{
	...
	public static class NeedBarOverflow_Patches
	{
		...
		public static MethodInfo PatchMethod(MethodBase original,
			Delegate prefix = null, Delegate postfix = null, Delegate transpiler = null)
		{
			HarmonyMethod preMethod = (prefix == null) ? null : new HarmonyMethod(prefix.Method);
			HarmonyMethod postMethod = (postfix == null) ? null : new HarmonyMethod(postfix.Method);
		...
		public static bool ShouldApplyPatch(int enableIdx, int sessionEnableIdx)
			=> ShouldApplyPatch(s.enabledA[enableIdx], sessionEnableIdx);
		public static bool ShouldApplyPatch(bool enabled, int sessionEnableIdx)
		{
		...
		public static void ApplyPatches()
		{
			...
			if (s.enabledA[C.Food])
			{
				if (ShouldApplyPatch(C.Food, C.FoodNutri))
					PatchMethod(typeof(Need_Food), nameof(Need_Food.NutritionWanted), mType: MethodType.Getter, prefix: (ActionRef<float>)N.NutritionWanted_Postfix);
				if (ShouldApplyPatch(s.enabledB[C.V(C.Food, 1)], C.FoodNoEat))
				{
					PatchMethod(typeof(FloatMenuMakerMap), "AddHumanlikeOrders", postfix: (Action<UnityEngine.Vector3, Pawn, List<FloatMenuOption>>)N.AddHumanlikeOrders_Postfix);
					PatchMethod(typeof(FoodUtility), nameof(FoodUtility.WillIngestFromInventoryNow), postfix: (ActionRef_r3<Pawn, Thing, bool>)N.WillIngestFromInventoryNow_Postfix);
				}
				if (ShouldApplyPatch(s.FoodOverflowAffectHealth, C.FoodHediff))
					PatchMethod(typeof(Need_Food), nameof(Need_Food.NeedInterval), transpiler: (Func_TranspilerILG)N.Food_NeedInterval_Transpiler);
			}
			//Rest Patches
			if (s.enabledA[C.Rest])
            {
				if (ShouldApplyPatch(s.enabledB[C.V(C.Rest, 1)], C.RestDrain))
                {
					...
					PatchMethod(typeof(Need_Rest), nameof(Need_Rest.NeedInterval), transpiler: (Func_Transpiler)N.Drain_NeedInterval_Transpiler);
				}
				if (ShouldApplyPatch(s.enabledB[C.V(C.Rest, 2)], C.RestGain))
				{
					S.patchParamInt[0] = C.Rest;
					PatchMethod(typeof(Need_Rest), nameof(Need_Rest.NeedInterval), transpiler: (Func_Transpiler)N.Gain_NeedInterval_Transpiler);
				}
			}
			//Joy Patches
			if (s.enabledA[C.Joy])
			{
				if (ShouldApplyPatch(C.Joy, C.JoyPatch))
					PatchMethod(typeof(Need_Joy), nameof(Need_Joy.GainJoy), transpiler: (Func_Transpiler)N.GainJoy_Transpiler);
				if (ShouldApplyPatch(s.enabledB[C.V(C.Joy, 1)], C.JoyDrain))
				{
					S.patchParamInt[0] = C.Joy;
					PatchMethod(typeof(Need_Joy), nameof(Need_Joy.NeedInterval), transpiler: (Func_Transpiler)N.Drain_NeedInterval_Transpiler);
				}
				if (ShouldApplyPatch(s.enabledB[C.V(C.Joy, 2)], C.JoyGain))
					PatchMethod(typeof(Need_Joy), nameof(Need_Joy.GainJoy), prefix: (ActionRef_r2<Need_Joy, float>)N.GainJoy_Prefix);
			}
			//Mood Patches
			if (ShouldApplyPatch(C.Mood, C.MoodPatch))
			{
				S.patchParamInt[0] = C.Mood;
				PatchMethod(typeof(Need_Mood), nameof(Need_Mood.CurInstantLevel), mType: MethodType.Getter, transpiler: (Func_Transpiler)N.Clamp01_Transpiler);
				PatchMethod(typeof(ColonistBarColonistDrawer), nameof(ColonistBarColonistDrawer.DrawColonist), transpiler: (Func_Transpiler)N.CurLevelPercentage_Transpiler);
				PatchMethod(typeof(InspectPaneFiller), "DrawMood", transpiler: (Func_Transpiler)N.CurLevelPercentage_Transpiler);
				PatchMethod(typeof(InspirationHandler), "StartInspirationMTBDays", mType: MethodType.Getter, transpiler: (Func_Transpiler)N.StartInspirationMTBDays_Transpiler);
			}
			//Beauty
			if (ShouldApplyPatch(C.Beauty, C.BeautyPatch))
			{
				S.patchParamInt[0] = C.Beauty;
				PatchMethod(typeof(Need_Beauty), "LevelFromBeauty", transpiler: (Func_Transpiler)N.Clamp01_Transpiler);
			}
			//Comfort
			if (ShouldApplyPatch(C.Comfort, C.ComfortPatch))
			{
				S.patchParamInt[0] = C.Comfort;
				PatchMethod(typeof(Need_Comfort), nameof(Need_Comfort.CurInstantLevel), mType: MethodType.Getter, transpiler: (Func_Transpiler)N.Clamp01_Transpiler);
			}
			//Outdoors
			if (ShouldApplyPatch(C.Outdoors, C.OutdoorsPatch))
				PatchMethod(typeof(Need_Outdoors), nameof(Need_Outdoors.NeedInterval), transpiler: (Func_Transpiler)N.RemoveLastMin_Transpiler);
#if (v1_3 || v1_4 || v1_5)
            //Indoors
            if (ShouldApplyPatch(C.Indoors, C.IndoorsPatch))
				PatchMethod(typeof(Need_Indoors), nameof(Need_Indoors.NeedInterval), transpiler: (Func_Transpiler)N.RemoveLastMin_Transpiler);
			//Suppression
			if (ShouldApplyPatch(C.Suppression, C.SuppressionPatch))
			{
				PatchMethod(typeof(Need_Suppression), nameof(Need_Suppression.DrawSuppressionBar), transpiler: (Func_Transpiler)N.CurLevelPercentage_Transpiler);
				PatchMethod(typeof(Need_Suppression), nameof(Need_Suppression.DrawSuppressionBar), transpiler: (Func_TranspilerILG)N.DrawSuppressionBar_Transpiler);
			}
#endif
#if (v1_4 || v1_5)
            //KillThirst
            if (s.enabledA[C.KillThirst])
			{
				if (ShouldApplyPatch(C.KillThirst, C.KillThirstPatch))
					PatchMethod(typeof(Need_KillThirst), nameof(Need_KillThirst.Notify_KilledPawn), transpiler: (Func_Transpiler)N.KillThirst_KilledPawn_Transpiler);
				if (ShouldApplyPatch(s.enabledB[C.V(C.KillThirst, 1)], C.KillThirstDrain))
				{
					S.patchParamInt[0] = C.KillThirst;
					PatchMethod(typeof(Need_KillThirst), nameof(Need_KillThirst.NeedInterval), transpiler: (Func_Transpiler)N.Drain_NeedInterval_Transpiler);
				}
			}
			//Need_Learning
			if (ShouldApplyPatch(C.Learning, C.LearnPatch))
			{
				PatchMethod(typeof(Need_Learning), nameof(Need_Learning.Learn), transpiler: (Func_Transpiler)N.Learn_Transpiler);
				PatchMethod(typeof(Gizmo_GrowthTier), "DrawLearning", transpiler: (Func_Transpiler)N.DrawLearning_Transpiler);
			}
			//Need_MechEnergy
			if (ShouldApplyPatch(C.MechEnergy, C.MechEnergyPatch))
				PatchMethod(typeof(InspectPaneFiller), "DrawMechEnergy", transpiler: (Func_Transpiler)N.CurLevelPercentage_Transpiler);
			//Need_Play
			if (ShouldApplyPatch(C.Play, C.PlayPatch))
				PatchMethod(typeof(Need_Play), nameof(Need_Play.Play), transpiler: (Func_Transpiler)N.Play_Transpiler);
#endif
		}
	}
}
