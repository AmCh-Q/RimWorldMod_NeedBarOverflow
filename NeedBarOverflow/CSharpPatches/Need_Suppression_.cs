...
namespace NeedBarOverflow.Patches.Need_Suppression_
{
	...
	public static class DrawSuppressionBar
	{
		...
		public static void Toggle()
			=> Toggle(PatchApplier.Enabled(Consts.Suppression));
			...
			IEnumerable<CodeInstruction> instructions, ILGenerator ilg)
		{
			...
			ReadOnlyCollection<CodeInstruction> instructionList = ClampCurLevelPercentage.Transpiler(instructions).ToList().AsReadOnly();
			int state = 0;
